<template>
  <div style="margin-top: 15px">
    <div class="form-item">
      <div class="input-box">
        <input v-model.trim="email" :placeholder="t('pleaseInputEmail')" autocomplete="off" />
        <div class="icon">
          <svg
            @click="close"
            t="1747822847376"
...
const getCode = debounce(() => {
  ...
  loading.value = true
  emailCode('LOGIN', email.value).then((res) => {
    if (res.code == '200') {
      loading.value = false
      showToast(res.msg)
      flag.value = true
...
const handleSubmit = () => {
  ...
  if (!showVerifyCode.value && !loginPassword.value) {
